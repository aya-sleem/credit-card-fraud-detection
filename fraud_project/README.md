...
.
├── data/
│   ├── download_data.py             # run this first — fetches the ~98MB dataset
│   ├── creditcard.csv               # raw dataset (downloaded, not committed — see .gitignore)
│   └── data_dictionary.md           # feature descriptions + evaluation rationale
├── notebooks/
│   └── analysis.ipynb               # full, executed analysis notebook (main deliverable)
├── src/
│   └── pipeline.py                  # equivalent standalone script version
├── results/
│   ├── figures/                     # EDA, ROC/PR curves, confusion matrices, feature importance
│   ├── code_images/                 # syntax-highlighted code screenshots
│   ├── model_comparison.csv         # final metrics table for both models
│   └── feature_importance.csv       # Random Forest feature importances
├── reports/
│   └── final_report.md              # written summary report (problem → conclusion)
├── requirements.txt
├── .gitignore
└── README.md
```
...
python3 data/download_data.py     # fetches the dataset (~98MB), only needed once
